...
save_path = fullfile(load_jrs_path,'jrs_trig_tensor_saved');
...
for j = 1:n_JRS
    ...
    filename = fullfile(save_path,sprintf('jrs_trig_tensor_mat_%0.3f.mat', current_c_kvi));
    
    % convert JRS zonotope to JRS matrix for the sake of python loading
    n_timestep = size(JRS,1);
    JRS_tensor = zeros(n_timestep,13,dim);
    figure(1); clf; hold on;
    for i = 1:n_timestep
        if i == 1 || i == 51
            JRS{i} = JRS{i}.reduce('girard',2);
            Z = JRS{i}.Z;
        else
            Z = JRS{i}.Z;
        end
        JRS_tensor(i,1:size(Z,2),:) = Z';
    ...
    save(filename, 'JRS_tensor', 'options', 't_plan', 't_total', 'current_c_kvi');
