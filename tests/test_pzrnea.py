...
class TestPZRNEA(unittest.TestCase):
    def setUp(self):
        ...
        self.robot = zpr.ZonoArmRobot.load(os.path.join(basedirname, 'robots/assets/robots/kinova_arm/gen3.urdf'), create_joint_occupancy=True)
        # initial conditions
        ...
        self.jrsgen = zpr.JrsGenerator(self.robot, traj_class=traj_class, ultimate_bound=0.0191, k_r=10)
        self.batchjrsgen = zpr.JrsGenerator(self.robot, traj_class=traj_class, ultimate_bound=0.0191, k_r=10, batched=True, unique_tid=False)
    ...
    def test_batch_singular_equality(self):
        out_batch = zpr.dynamics.pzrnea(self.batchjrs['R'], self.batchjrs['qd'], self.batchjrs['qd_aux'], self.batchjrs['qdd_aux'], self.robot)
        out_0 = zpr.dynamics.pzrnea(self.jrs['R'][0], self.jrs['qd'][0], self.jrs['qd_aux'][0], self.jrs['qdd_aux'][0], self.robot)
        out_20 = zpr.dynamics.pzrnea(self.jrs['R'][20], self.jrs['qd'][20], self.jrs['qd_aux'][20], self.jrs['qdd_aux'][20], self.robot)
        out_75 = zpr.dynamics.pzrnea(self.jrs['R'][75], self.jrs['qd'][75], self.jrs['qd_aux'][75], self.jrs['qdd_aux'][75], self.robot)
        out_99 = zpr.dynamics.pzrnea(self.jrs['R'][99], self.jrs['qd'][99], self.jrs['qd_aux'][99], self.jrs['qdd_aux'][99], self.robot)
    ...
    def test_known_centers(self):
        ...
        out_batch = zpr.dynamics.pzrnea(center_R, center_qd, center_qd_aux, center_qdd_aux, self.robot)
