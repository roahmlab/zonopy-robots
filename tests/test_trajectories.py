...
# Helper
rem_dep = np.vectorize(zp.remove_dependence_and_compress, excluded=[1])
...
class TestPiecewiseArmTrajectories(unittest.TestCase):
    ...
    def test_construction3link(self):
        zpr.trajectory.PiecewiseArmTrajectory(self.q, self.dq, None, self.params, self.param_range, tbrake=0.5, tfinal=1)

    def test_generation3link(self):
        gen = zpr.trajectory.PiecewiseArmTrajectory(self.q, self.dq, None, self.params, self.param_range, tbrake=0.5, tfinal=1)
    ...
    def test_generation3link_batch(self):
        gen = zpr.trajectory.PiecewiseArmTrajectory(self.q, self.dq, None, self.params, self.param_range)
...
class TestBernsteinArmTrajectories(unittest.TestCase):
    ...
    def test_construction3link(self):
        zpr.trajectory.BernsteinArmTrajectory(self.q, self.dq, self.q, self.params, self.param_range, tbrake=0.5, tfinal=1)

    def test_generation3link(self):
        gen = zpr.trajectory.BernsteinArmTrajectory(self.q, self.dq, self.q, self.params, self.param_range, tbrake=0.5, tfinal=1)
        out = gen.getReference(np.array([self.times[i] for i in range(self.times.batch_shape[0])]))
    ...
    def test_generation3link_batch(self):
        gen = zpr.trajectory.BernsteinArmTrajectory(self.q, self.dq, self.q, self.params, self.param_range)
        out = gen.getReference(self.times)
